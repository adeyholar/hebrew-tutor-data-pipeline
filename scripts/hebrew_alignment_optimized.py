# File location: D:\AI\Gits\hebrew-tutor-data-pipeline\scripts\hebrew_alignment_production.py
# Hebrew Tutor App - Audio-Text Force Alignment Pipeline (Production Optimized)
# Optimized version with batch processing, improved error handling, and performance enhancements
...
class HebrewAlignmentTool:
    def __init__(self):
        ...
        logger.info("=== INITIALIZING HEBREW ALIGNMENT TOOL (PRODUCTION) ===")
        ...
        if self.device.type == "cuda" and self.gpu_usable:
            # GPU settings - optimized for production
            self.model_name = "openai/whisper-medium"  # Better accuracy for Hebrew
            self.batch_size = 8
            self.chunk_length = 30
            self.max_length = 448
            self.enable_batch_processing = True
            logger.info("🚀 GPU mode: Production settings with whisper-medium")
        else:
            # CPU settings
        ...
        # Audio processing settings with optimization
        self.sample_rate = 16000
        self.audio_cache = {}  # Cache for frequently accessed audio
    ...
    def _init_azure(self):
        ...
        try:
            ...
            if connection_string:
                ...
                logger.info("ℹ️ Azure Blob Storage not configured (optional)")
            ...
            if speech_key:
                ...
                logger.info("ℹ️ Azure Speech SDK not configured (optional)")
                
        except Exception as e:
            logger.warning(f"⚠️ Azure initialization partial failure: {e}")
    ...
    def _lazy_load_whisper(self):
        """Lazy load Whisper model with optimization."""
    ...
    def extract_verses(self, book_code: str, chapter: int) -> List[Dict]:
        ...
        try:
            ...
            # Optimized book mapping - try direct lookup first
            json_book_key = None
    ...
    async def load_audio_optimized(self, audio_path: Path) -> Tuple[Optional[np.ndarray], int]:
        ...
        try:
            ...
            logger.warning(f"⚠️ MP3 decode issue (attempting fix): {e}")
            try:
    ...
    async def transcribe_with_whisper(self, audio_path: Path, retries: int = 0) -> Tuple[bool, List[Dict]]:
        """Transcribe audio with enhanced GPU optimization."""
        ...
        try:
            ...
            # Process with Whisper (optimized)
            ...
            generation_kwargs = {
                ...
                "num_beams": 1,
                "do_sample": False,
                "language": "hebrew",  # Explicit Hebrew language
    ...
    def save_alignment_json(self, book_code: str, chapter: int, alignment_data: Dict) -> bool:
        """Save alignment data with atomic write operation."""
        try:
    ...
    async def process_chapter(self, book_code: str, chapter: int) -> bool:
        ...
        try:
            ...
            if whisper_success and segments:
                ...
                logger.warning("⚠️ Whisper failed, creating estimates")
    ...
    async def process_book(self, book_code: str) -> bool:
        ...
        logger.info(f"⏱️ Total time: {book_processing_time/60:.1f} minutes, Avg per chapter: {avg_chapter_time:.1f}s")
    ...
    async def process_all(self) -> bool:
        ...
        logger.info(f"⏱️ Total time: {total_time/3600:.1f} hours, Avg per chapter: {self.performance.get_average_time():.1f}s")
...
async def main():
    ...
    try:
        ...
        if len(sys.argv) == 1:
            ...
            book_code = sys.argv[2]
            start_chapter = int(sys.argv[3])
            end_chapter = int(sys.argv[4])
            logger.info(f"📦 Batch processing: {book_code} chapters {start_chapter}-{end_chapter}")
            # Implementation would go here
        else:
            print("Usage:")
            print("  python hebrew_alignment_production.py                    # Process all")
            print("  python hebrew_alignment_production.py test Gen           # Test book")
            print("  python hebrew_alignment_production.py test Gen 1         # Test chapter")
            print("  python hebrew_alignment_production.py batch Gen 1 10     # Batch chapters (future)")
