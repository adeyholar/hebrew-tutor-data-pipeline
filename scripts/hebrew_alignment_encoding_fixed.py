# Hebrew Tutor App - Audio-Text Force Alignment Pipeline (ENCODING FIXED)
# Fixed Hebrew text encoding issues based on converter.py approach
...
class HebrewAlignmentTool:
    def __init__(self):
        """Initialize the Hebrew Alignment Tool with FIXED encoding."""
        logger.info("=== INITIALIZING HEBREW ALIGNMENT TOOL (ENCODING FIXED) ===")
    ...
    def get_audio_file_path(self, book_code: str, chapter: int) -> Optional[Path]:
        ...
        # FIXED: Handle both input formats (Gen/Genesis style and Exod/Exodus style)
        audio_book_code = book_code
        if book_code in ['Exo', 'Deu', 'Jos', 'Jdg', '1Sa', '2Sa', '1Ki', '2Ki', 
                        '1Ch', '2Ch', 'Est', 'Psa', 'Pro', 'Ecc', 'Sol', 'Eze', 
                        'Joe', 'Amo', 'Oba', 'Jon', 'Hab', 'Zep', 'Zec']:
            # Map old-style codes to new format for audio lookup
            old_to_new = {
            ...
            if book_code in old_to_new:
                audio_book_code = old_to_new[book_code]
        ...
        # FIXED: Use original book_code for filename (Gen not Genesis)
    ...
    def extract_verses(self, book_code: str, chapter: int) -> List[Dict]:
        ...
        try:
            ...
            # FIXED: Use the exact book keys from your converter.py
            ...
            if not isinstance(book_data, list) or chapter < 1 or chapter > len(book_data):
                logger.error(f"Invalid chapter {chapter} for book {json_book_key} (has {len(book_data)} chapters)")
                return []
            ...
            # FIXED: Log first verse to verify Hebrew encoding
            if formatted_verses:
                ...
                logger.debug(f"First verse Hebrew chars: {hebrew_chars}/{len(first_verse)}: '{first_verse[:50]}...'")
    ...
    def save_alignment_json(self, book_code: str, chapter: int, alignment_data: Dict) -> bool:
        """FIXED: Save alignment data with proper UTF-8 encoding like converter.py."""
        try:
            ...
            try:
                # FIXED: Write with UTF-8 encoding and ensure_ascii=False like converter.py
    ...
    async def process_book(self, book_code: str) -> bool:
        ...
        # Get chapter count from Bible data
        if book_code not in self.bible_data:
...
async def main():
    ...
    try:
        ...
        if len(sys.argv) == 1:
            ...
            print("Usage:")
            print("  python hebrew_alignment_encoding_fixed.py                    # Process all")
            print("  python hebrew_alignment_encoding_fixed.py test Gen           # Test book")
            print("  python hebrew_alignment_encoding_fixed.py test Gen 1         # Test chapter")
