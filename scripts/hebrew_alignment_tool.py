...
Uses GPU-accelerated OpenAI Whisper for primary alignment, fallback to Azure Speech
...
class HebrewBibleAligner:
    def __init__(self):
        ...
        # Initialize Whisper with GPU and tokenizer
        self.model = AutoModelForSpeechSeq2Seq.from_pretrained("openai/whisper-medium", torch_dtype=torch.float16).to(device)
        ...
        self.whisper_pipe = pipeline(
            ...
            device=device,
            batch_size=8  # Adjust for 8GB VRAM
        )
    ...
    async def _transcribe(self, audio_file: Path, reference_text: str) -> Optional[Dict]:
        """Primary: GPU-accelerated Whisper, Fallback: Azure"""
        try:
            # Whisper transcription with GPU
            result = self.whisper_pipe(str(audio_file), return_timestamps="word", chunk_length_s=30, batch_size=8)
            processed_words = []
            total_confidence = 0
            word_count = 0
            total_duration = 0
            ...
            for chunk in result['chunks']:
                for w in chunk['text'].split():  # Refine splitting if needed
                    ...
                    word_count += 1
                    total_duration = max(total_duration, chunk['timestamp'][1])
    ...
    async def _transcribe_with_azure(self, audio_file: Path, reference_text: str) -> Optional[Dict]:
        try:
            ...
            logger.info(f"Starting Azure transcription for {audio_file.name}")
