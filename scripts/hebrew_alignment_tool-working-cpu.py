...
# Hebrew Tutor App - Audio-Text Force Alignment Pipeline
# Updated script with CPU optimization and improved error handling
...
class HebrewAlignmentTool:
    def __init__(self):
        """Initialize the Hebrew Alignment Tool with CPU-optimized settings."""
        # Force CPU mode for stability as per user preference
        self.device = torch.device("cpu")
        logger.info(f"Using device: {self.device}")
        
        # Whisper model setup - using small model for CPU efficiency
        self.model_name = "openai/whisper-small"
        self.processor = None
        self.model = None
        
        # Audio processing settings - optimized for CPU
        self.sample_rate = 16000
        self.batch_size = 2  # Reduced for CPU
        self.chunk_length = 15  # Reduced chunk length for CPU
        ...
        self.json_path = self.base_path / "book" / "hebrew_bible_with_nikkud.json"  # Fixed path
    ...
    async def download_audio_from_blob(self, book_code: str, chapter: int) -> Optional[Path]:
        """Download audio file from Azure Blob Storage if not available locally."""
        if not self.blob_service_client:
            return None
            
        try:
            # Construct blob path
            folder_map = {'Gen': '01_Genesis'}  # Add more as needed
            if book_code not in folder_map:
                return None
                
            blob_name = f"audio/{folder_map[book_code]}/{book_code}_{chapter:02d}.mp3"
            ...
            blob_client = self.blob_service_client.get_blob_client(
                container="hebrew-audio", blob=blob_name
            )
            ...
            logger.info(f"Downloaded audio from blob: {blob_name}")
            return temp_path
            
        except Exception as e:
            logger.error(f"Failed to download from blob: {e}")
            return None
    ...
    async def transcribe_with_whisper(self, audio_path: Path, retries: int = 0) -> Tuple[bool, List[Dict]]:
        """Transcribe audio using Whisper with retry mechanism."""
        try:
            ...
            # Generate with forced timestamps
            with torch.no_grad():
                predicted_ids = self.model.generate(
                    inputs["input_features"],
                    return_timestamps=True,
                    max_length=448,  # Reduced for CPU
            ...
            # Parse timestamps - improved error handling
            segments = []
            if transcription and len(transcription) > 0:
                text = transcription[0]
                # Simple timestamp parsing - can be enhanced
                segments.append({
                    'text': text,
                    'start': 0.0,
            ...
            logger.error(f"Whisper transcription failed (attempt {retries + 1}): {e}")
            if retries < self.max_retries - 1:
                await asyncio.sleep(self.retry_delay)
                return await self.transcribe_with_whisper(audio_path, retries + 1)
            return False, []
    
    async def transcribe_with_azure(self, audio_path: Path) -> Tuple[bool, List[Dict]]:
        """Fallback transcription using Azure Speech SDK."""
        if not self.speech_config:
            return False, []
            
        temp_wav_path = None
        try:
            # Convert MP3 to WAV for Azure
            temp_wav_file = tempfile.NamedTemporaryFile(suffix='.wav', delete=False)
            temp_wav_path = Path(temp_wav_file.name)
            temp_wav_file.close()
            
            # Load and convert using librosa and soundfile
            audio_data, sr = librosa.load(str(audio_path), sr=16000)
            sf.write(str(temp_wav_path), audio_data, sr)
            
            # Configure Azure recognizer
            audio_config = speechsdk.audio.AudioConfig(filename=str(temp_wav_path))
            ...
            if result.reason == speechsdk.ResultReason.RecognizedSpeech:
                ...
                logger.info("Azure transcription successful")
                return True, segments
            else:
    ...
    async def process_chapter(self, book_code: str, chapter: int) -> bool:
        ...
        try:
            ...
            if not audio_path or not audio_path.exists():
                ...
                audio_path = await self.download_audio_from_blob(book_code, chapter)
            ...
            if whisper_success and segments:
                ...
                # Try Azure fallback
                azure_success, segments = await self.transcribe_with_azure(audio_path)
                if azure_success and segments:
                    alignment_data = self.fuzzy_align_text(segments, verses)
                    success = True
                    logger.info("Azure transcription successful")
                else:
                    # Create estimated alignment
                    logger.warning("Both transcription methods failed, creating estimates")
    ...
    async def process_book(self, book_code: str) -> bool:
        ...
        if chapter_count == 0:
            logger.error(f"No chapters found for book: {book_code}")
            return False
    ...
    async def process_all(self) -> bool:
        ...
        book_codes = list(set([book.get('book_code') for book in self.bible_data if book.get('book_code')]))
        book_codes.sort()
...
async def main():
    ...
    if len(sys.argv) == 1:
        ...
        print("Usage:")
        print("  python hebrew_alignment_tool.py                    # Process all")
        print("  python hebrew_alignment_tool.py test Gen           # Test book")
        print("  python hebrew_alignment_tool.py test Gen 1         # Test chapter")
